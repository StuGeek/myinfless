...
func MakeDeleteHandler(defaultNamespace string, clientset *kubernetes.Clientset) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		podList, findPodsErr := clientset.CoreV1().Pods(lookupNamespace).List(context.TODO(), listPodOptions)
		if findPodsErr != nil {
		...
		srvsList, findServErr := clientset.CoreV1().Services(lookupNamespace).List(context.TODO(), listOpts)
		if findServErr != nil {
...
func deleteFunctionPod(functionNamespace string, listPodOptions metav1.ListOptions, clientset *kubernetes.Clientset) error {
	...
	deletePodsErr := clientset.CoreV1().Pods(functionNamespace).DeleteCollection(context.TODO(), opts, listPodOptions)
...
func deleteFunctionService(functionNamespace string, listPodOptions metav1.ListOptions, clientset *kubernetes.Clientset, functionName string) error {
	...
	svcErr := clientset.CoreV1().Services(functionNamespace).Delete(context.TODO(), functionName, opts)
	if svcErr != nil {
		return svcErr
	}
