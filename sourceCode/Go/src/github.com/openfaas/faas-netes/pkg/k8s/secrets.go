...
func (f *FunctionFactory) ConfigureSecrets(request types.FunctionDeployment, pod *apiv1.Pod, existingSecrets map[string]*apiv1.Secret) error {
	...
	// added below
	existingVolumes := removeVolume(volumeName, pod.Spec.Volumes)
	pod.Spec.Volumes = existingVolumes
	if len(secretVolumeProjections) > 0 {
		pod.Spec.Volumes = append(existingVolumes, projectedSecrets)
	}
