...
func createFuncInstance(funcName string, namespace string, funcPodConfig *gpuTypes.FuncPodConfig, podType string, clientset *kubernetes.Clientset) error {
	...
	* build the cuda device env str
	 ...
	 * create pod
	 */
	_, err := clientset.CoreV1().Pods(namespace).Create(context.TODO(), funcDeployStatus.FuncSpec.Pod, metav1.CreateOptions{})
	if err != nil {
	...
	for {
		...
		pods, podErr := clientset.CoreV1().Pods(namespace).Get(context.TODO(), podName, metav1.GetOptions{})
		if podErr != nil {
	...
	*/
	if ip == "" {
		...
		delError := clientset.CoreV1().Pods(namespace).Delete(context.TODO(), podName, opts)
		if delError != nil {
