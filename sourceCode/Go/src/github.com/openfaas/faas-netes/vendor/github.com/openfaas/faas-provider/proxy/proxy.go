...
func proxyRequest(w http.ResponseWriter, originalReq *http.Request, proxyClient *http.Client, resolver BaseURLResolver) {
	...
	originalReq.Header["Content-Type"] = []string{"application/json"}
	...
	proxyReq, err := buildProxyRequest(originalReq, functionAddr, functionAddr.Path)
	...
	response, errors := proxyClient.Do(proxyReq.WithContext(ctx))
	if errors != nil {
		log.Printf("error with proxy request to: %s, %s\n", proxyReq.URL.String(), errors.Error())
