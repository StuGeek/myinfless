...
func runController(setup serverSetup) {
	...
	operator := false
	startInformers(setup, stopCh, operator)
	...
	bootstrapHandlers := providertypes.FaaSHandlers{
		...
		DeployHandler:        handlers.MakeDeployHandler(config.DefaultFunctionNamespace, factory, kubeClient),
	...
	go aside.RpsDispatcherMonitor(config.DefaultFunctionNamespace, config.LoadGenHost, config.LoadGenPort)
	cpuRepository.InitializeCluster(kubeClient)
