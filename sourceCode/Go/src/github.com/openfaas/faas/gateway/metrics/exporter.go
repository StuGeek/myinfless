...
func (e *Exporter) Describe(ch chan<- *prometheus.Desc) {
	...
	e.metricOptions.ServiceExpectReplicasGauge.Describe(ch)
	e.metricOptions.ServiceAvailReplicasGauge.Describe(ch)
}
...
func (e *Exporter) Collect(ch chan<- prometheus.Metric) {
	...
	e.metricOptions.ServiceExpectReplicasGauge.Reset()
	e.metricOptions.ServiceAvailReplicasGauge.Reset()
	for _, service := range e.services {
		...
		e.metricOptions.ServiceExpectReplicasGauge.
			WithLabelValues(serviceName).
			Set(float64(service.Replicas))
		e.metricOptions.ServiceAvailReplicasGauge.
			WithLabelValues(serviceName).
			Set(float64(service.AvailableReplicas))
	}
	e.metricOptions.ServiceReplicasGauge.Collect(ch)

	e.metricOptions.ServiceExpectReplicasGauge.Collect(ch)
	e.metricOptions.ServiceAvailReplicasGauge.Collect(ch)
}
