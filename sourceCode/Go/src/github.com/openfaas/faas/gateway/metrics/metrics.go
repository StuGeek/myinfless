...
type MetricOptions struct {
	...
	ServiceExpectReplicasGauge      *prometheus.GaugeVec
	ServiceAvailReplicasGauge      *prometheus.GaugeVec
}
...
func BuildMetricsOptions() MetricOptions {
	...
	serviceExpectReplicas := prometheus.NewGaugeVec(
		prometheus.GaugeOpts{
			Name: "gateway_expect_service_count",
			Help: "Docker service expected replicas",
		}, []string{"function_name"},
	)

	serviceAvailReplicas := prometheus.NewGaugeVec(
		prometheus.GaugeOpts{
			Name: "gateway_avail_service_count",
			Help: "Docker service available replicas",
		}, []string{"function_name"},
	)
	...
	metricsOptions := MetricOptions{
		...
		ServiceExpectReplicasGauge:       serviceExpectReplicas,
		ServiceAvailReplicasGauge:        serviceAvailReplicas,
	}
