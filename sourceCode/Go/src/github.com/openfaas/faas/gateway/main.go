...
func main() {
	...
	if config.ScaleFromZero {
		log.Println("Scaling from zero enabled: Using cold start mechanism")
		...
		log.Println("Scaling from zero disabled: Not using cold start mechanism")
	}
	...
	if config.NATS {
		...
		log.Println("Async disabled: Not using NATS Streaming.")
	}
	...
	go runMetricsServer(config.MetricsServerPort)
	...
	s := &http.Server {
		Addr:           fmt.Sprintf(":%d", config.GatewayTcpPort),
...
func runMetricsServer(metricsServerPort int) {
