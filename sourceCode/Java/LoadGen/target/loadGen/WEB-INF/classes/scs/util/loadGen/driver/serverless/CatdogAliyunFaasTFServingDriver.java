...
import scs.util.loadGen.threads.LoadExecThreadAliyunCatdog;
 ...
 *
 */
public class CatdogAliyunFaasTFServingDriver extends AbstractJobDriver{
	/**
	 * Singleton code block
	 */
	private static CatdogAliyunFaasTFServingDriver driver=null;	
	
	//private StringBuilder builder=new StringBuilder();
	
	public CatdogAliyunFaasTFServingDriver(){initVariables();}
	public synchronized static CatdogAliyunFaasTFServingDriver getInstance() {
		if (driver == null) {
			driver = new CatdogAliyunFaasTFServingDriver();
		}
		return driver;
	}
	...
	protected void initVariables() {
		...
		queryItemsStr=Repository.catdogAliyunFaasBaseURL;
		jsonParmStr=Repository.catdogAliyunParmStr;
			...
			if(Repository.sendFlag[serviceId]==true){
				...
				if (Repository.realRequestIntensity[serviceId]==0){
					...
					for (int i=0;i<Repository.realRequestIntensity[serviceId];i++){ 
						executor.execute(new LoadExecThreadAliyunCatdog(httpClient,queryItemsStr,begin,serviceId,jsonParmStr,sleepUnit*i));
					}
				}
