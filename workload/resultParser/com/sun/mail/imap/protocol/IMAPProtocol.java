...
public class IMAPProtocol extends Protocol
{
    ...
    public synchronized void authntlm(final String authzid, final String u, final String p) throws ProtocolException {
        ...
        Response r = null;
        boolean done = false;
        final Ntlm ntlm = new Ntlm(this.debug ? this.out : null);
        int flags = 0;
        String domain = null;
        String type1Msg = null;
        try {
            flags = PropUtil.getIntProperty(this.props, "mail." + this.name + ".auth.ntlm.flags", 0);
            final boolean useUnicode = PropUtil.getBooleanProperty(this.props, "mail." + this.name + ".auth.ntlm.unicode", true);
            domain = this.props.getProperty("mail." + this.name + ".auth.ntlm.domain", "");
            type1Msg = ntlm.generateType1Msg(useUnicode, flags, domain, this.getLocalHost());
            if (type1Msg == null) {
                if (this.debug) {
                    this.out.println("IMAP DEBUG: Can't load NTLM authenticator");
                }
            ...
            try {
                ...
                if (r.isContinuation()) {
                    ...
                    if (first) {
                        ...
                        final int lmCompatibility = PropUtil.getIntProperty(this.props, "mail." + this.name + ".auth.ntlm.lmcompat", 3);
                        s = ntlm.generateType3Msg(u, p, domain, this.getLocalHost(), r.getRest(), flags, lmCompatibility);
                    }
    ...
    public void startTLS() throws ProtocolException {
        try {
            ...
            if (this.debug) {
                this.out.println("IMAP DEBUG: STARTTLS ProtocolException: " + pex);
            }
            throw pex;
        }
        catch (Exception ex) {
            if (this.debug) {
                this.out.println("IMAP DEBUG: STARTTLS Exception: " + ex);
            }
