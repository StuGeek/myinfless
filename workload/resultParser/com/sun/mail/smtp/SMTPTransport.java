...
public class SMTPTransport extends Transport
{
    ...
    protected boolean protocolConnect(String host, int port, final String user, final String passwd) throws MessagingException {
        ...
        if ((useAuth || (user != null && passwd != null)) && (this.supportsExtension("AUTH") || this.supportsExtension("AUTH=LOGIN"))) {
            ...
            while (st.hasMoreTokens()) {
                final String m = st.nextToken().toUpperCase(Locale.ENGLISH);
    ...
    private abstract class Authenticator
    {
        ...
        boolean authenticate(final String host, final String user, final String passwd) throws MessagingException {
            try {
                final String ir = this.getInitialResponse(host, user, passwd);
                if (ir != null) {
        ...
        String getInitialResponse(final String host, final String user, final String passwd) throws MessagingException, IOException {
            return null;
        }
        
        abstract void doAuth(final String p0, final String p1, final String p2) throws MessagingException, IOException;
    }
    ...
    private class LoginAuthenticator extends Authenticator
    {
        ...
        void doAuth(final String host, final String user, final String passwd) throws MessagingException, IOException {
    ...
    private class PlainAuthenticator extends Authenticator
    {
        ...
        void doAuth(final String host, final String user, final String passwd) throws MessagingException, IOException {
            ...
            b64os.flush();
            this.resp = SMTPTransport.this.simpleCommand(bos.toByteArray());
        }
    }
    ...
    private class NtlmAuthenticator extends Authenticator
    {
        ...
        private synchronized Ntlm getNtlm() {
            if (this.ntlm == null) {
                this.ntlm = new Ntlm(SMTPTransport.this.debug ? SMTPTransport.this.out : null);
            }
            return this.ntlm;
        }
        
        String getInitialResponse(final String host, final String user, final String passwd) throws MessagingException, IOException {
            final Ntlm ntlm = this.getNtlm();
            ...
            final boolean useUnicode = PropUtil.getBooleanProperty(SMTPTransport.this.session.getProperties(), "mail." + SMTPTransport.this.name + ".auth.ntlm.unicode", true);
            final String type1 = ntlm.generateType1Msg(useUnicode, this.flags, SMTPTransport.this.getNTLMDomain(), SMTPTransport.this.getLocalHost());
            return type1;
        }
        
        void doAuth(final String host, final String user, final String passwd) throws MessagingException, IOException {
            final int lmCompatibility = PropUtil.getIntProperty(SMTPTransport.this.session.getProperties(), "mail." + SMTPTransport.this.name + ".auth.ntlm.lmcompat", 3);
            final String type3 = this.ntlm.generateType3Msg(user, passwd, SMTPTransport.this.getNTLMDomain(), SMTPTransport.this.getLocalHost(), SMTPTransport.this.getLastServerResponse().substring(4), this.flags, lmCompatibility);
