...
public class Document extends Element
{
    ...
    private void normaliseStructure(final String tag, final Element htmlEl) {
        ...
        if (elements.size() > 1) {
            ...
            final Iterator<Node> iterator = toMove.iterator();
            while (iterator.hasNext()) {
                final Node dupe = iterator.next();
    ...
    private Element findFirstElementByTagName(final String tag, final Node node) {
        ...
        for (int size = node.childNodeSize(), i = 0; i < size; ++i) {
            final Element found = this.findFirstElementByTagName(tag, node.childNode(i));
            if (found != null) {
                return found;
            }
        }
    ...
    public static class OutputSettings implements Cloneable
    {
        ...
        public OutputSettings() {
            ...
            this.encoderThreadLocal = new ThreadLocal<CharsetEncoder>();
        ...
        CharsetEncoder encoder() {
            final CharsetEncoder encoder = this.encoderThreadLocal.get();
