...
public class PoolingHttpClientConnectionManager implements HttpClientConnectionManager, ConnPoolControl<HttpRoute>, Closeable
{
    ...
    public PoolingHttpClientConnectionManager(final HttpClientConnectionOperator httpClientConnectionOperator, final HttpConnectionFactory<HttpRoute, ManagedHttpClientConnection> connFactory, final long timeToLive, final TimeUnit tunit) {
        ...
        (this.pool = new CPool(new InternalConnectionFactory(this.configData, connFactory), 2, 20, timeToLive, tunit)).setValidateAfterInactivity(2000);
    ...
    public ConnectionRequest requestConnection(final HttpRoute route, final Object state) {
        ...
        return new ConnectionRequest() {
            ...
            public HttpClientConnection get(final long timeout, final TimeUnit tunit) throws InterruptedException, ExecutionException, ConnectionPoolTimeoutException {
                final HttpClientConnection conn = PoolingHttpClientConnectionManager.this.leaseConnection(future, timeout, tunit);
                if (conn.isOpen()) {
                    HttpHost host;
    ...
    public Set<HttpRoute> getRoutes() {
        return ((AbstractConnPool<HttpRoute, C, E>)this.pool).getRoutes();
    }
