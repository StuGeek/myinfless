...
public class HeaderTokenizer
{
    ...
    Token next(final char endOfAtom) throws ParseException {
        this.currentPos = this.nextPos;
        final Token tk = this.getNext(endOfAtom);
        final int currentPos = this.currentPos;
        this.peekPos = currentPos;
        this.nextPos = currentPos;
        return tk;
    ...
    private Token getNext(final char endOfAtom) throws ParseException {
            ...
            while (this.currentPos < this.maxPos) {
                ...
                if (c < ' ' || c >= '\u007f' || c == '(' || c == ' ' || c == '\"' || this.delimiters.indexOf(c) >= 0) {
                    if (endOfAtom > '\0' && c != endOfAtom) {
                        ...
                        return this.collectString(endOfAtom);
                    }
                    break;
                }
                else {
    ...
    private Token collectString(final char eos) throws ParseException {
        boolean filter = false;
        final int start = this.currentPos;
        while (this.currentPos < this.maxPos) {
        ...
        String s2;
        if (filter) {
            s2 = filterToken(this.string, start, this.currentPos);
        }
        else {
