...
public class MethodUtils
{
                ...
                if (methods[i].getName().equals(methodName)) {
                    ...
                    if (methodParamSize == paramSize) {
                        ...
                        if (match) {
                            ...
                            if (method2 != null) {
                                if (log.isTraceEnabled()) {
                                    log.trace(method2 + " accessible version of " + methods[i]);
                                }
    ...
    private static Method getCachedMethod(final MethodDescriptor md) {
        if (MethodUtils.CACHE_METHODS) {
            final Reference methodRef = (Reference)MethodUtils.cache.get(md);
            if (methodRef != null) {
    ...
    private static void cacheMethod(final MethodDescriptor md, final Method method) {
        if (MethodUtils.CACHE_METHODS && method != null) {
            MethodUtils.cache.put(md, new WeakReference(method));
        }
    }
    ...
    static {
        ...
        (cache = new WeakFastHashMap()).setFast(true);
    }
